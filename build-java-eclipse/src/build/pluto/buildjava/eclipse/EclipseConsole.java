...
public class EclipseConsole {
  ...
  private static final String CONSOLE_NAME = "Java cleardep builder console";
  ...
  private synchronized static MessageConsole getConsole() {
    ...
    for (IConsole console : consoles.getConsoles()) {
      if (EclipseConsole.CONSOLE_NAME.equals(console.getName()))
        return (MessageConsole) console;
    }
    // No console found, so create a new one
    MessageConsole result = new MessageConsole(EclipseConsole.CONSOLE_NAME, null);
  ...
  public static void activateConsole(final boolean consoleViewOnly) {
    Job job = new UIJob("Open console") {
      ...
      public IStatus runInUIThread(IProgressMonitor monitor) {
        ...
        MessageConsole console = EclipseConsole.getConsole();
        if (consoleViewOnly) {
          console.activate();
        ...
        try {
          ...
          Log.err.println("Could not activate the console: " + e.getMessage());
        }
        return Status.OK_STATUS;
      }
    };
